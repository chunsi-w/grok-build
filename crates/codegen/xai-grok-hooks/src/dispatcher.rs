...
struct SequentialGateOutcome {
    ...
    /// Concatenated soft-warn / `additionalContext` from allowing hooks
    /// (hookify `warn`). `None` when no hook produced context.
...
async fn dispatch_sequential_gate(
    ...
    SequentialGateOutcome {
        block: None,
        updated_input,
        soft_warn: if warn_contexts.is_empty() {
            None
        } else {
...
pub async fn dispatch_pre_tool_use(
    ...
    PreToolUseResult {
        decision: match outcome.block {
            ...
            None => match outcome.soft_warn {
                Some(ctx) => HookDecision::allow_with_context(ctx),
    ...
    for spec in hooks {
        ...
        match result {
            ...
            HookRunnerResult::Block { reason, .. } => {
                // Observe gate never hard-blocks; surface the reason as
                // soft-warn so PostToolUse audits still reach the model.
                warn_contexts.push(reason);
                tracing::info!(
                    hook_name = %spec.name,
                    elapsed_ms = elapsed.as_millis() as u64,
