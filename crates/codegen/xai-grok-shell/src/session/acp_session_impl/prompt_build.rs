...
impl SessionActor {
    ...
    pub(super) async fn build_user_message_prefix(&self) -> String {
        ...
        let mut out = if !matches!(template, UserMessageTemplate::Default) {
                ...
                .await
            {
                ...
                self.construct_legacy_prefix(cwd, language.as_deref())
            }
        } else {
            self.construct_legacy_prefix(cwd, language.as_deref())
        };
    ...
    fn construct_legacy_prefix(&self, cwd: &std::path::Path, language: Option<&str>) -> String {
        construct_user_message_minimal(cwd, None, language)
    }
